...
client = Client(None, client_utils)
...
def client_leaderboard(
    ...
    try:
        result = client_utils.get_leaderboard(all, include_alumni, id)
        if result.is_err():
            print(result)
...
def client_stats(
    ...
    if get_submissions:
        client_utils.get_submissions(client.vid)
        return

    if get_runs_for_submission is not None:
        temp: Result = client_utils.get_runs_for_submission(get_runs_for_submission, client.vid)
        if temp.is_err():
            print(temp.Err)
        return

    if get_code_for_submission is not None:
        temp: Result = client_utils.get_code_from_submission(get_code_for_submission, client.vid)
        if temp.is_err():
            print(temp.Err)
        return

    if get_details_for_submission is not None:
        temp: Result = client_utils.get_submission_run_info(get_details_for_submission, client.vid)
        if temp.is_err():
            print(temp.Err)
        return
